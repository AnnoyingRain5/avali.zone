...
def create_golink(userinfo, user):
    if request.method == "GET":
        golinks = (
            db.get_db()
            .execute(
                "SELECT name, destination FROM golinks WHERE owner = ?",
                (f"GOOGLE_{userinfo['email']}",),
            )
            .fetchall()
        )
        return render_template(
            "create_golink.jinja", userinfo=userinfo, golinks=golinks
        )
    else:
        # it's a POST request!
...
def _go(golink: str):
    ...
    golink = database.execute(
        "SELECT * FROM golinks WHERE name = ?", (golink,)
    ).fetchone()
    if golink:
